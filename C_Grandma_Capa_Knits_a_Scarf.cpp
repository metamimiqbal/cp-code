...
ll calc(string s, ll l, ll r, char c) {
    ll n = sz(s);
    ll cn = 0;
...
void solve() {
    ...
    if(l == -1) cout<<0<<nl; 
        ...
        //function (s, l, r, character)
        ll x = calc(s, l, r, ca);
        ll y = calc(s, l, r, cb);
        if((!x) && (!y)) cout<<-1<<nl; 
        else if((x) == -1ll) cout<<y<<nl;
        else if((y) == -1ll) cout<<x<<nl; 
        else cout<<min(x, y)<<nl;
    }
}
