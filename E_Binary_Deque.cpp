...
void solve() {
    ...
    ll ans = inf;
    rep(i, 1, n+1) {
        ll l = i, r = n, prospective = -1;
        while(l<=r) { // ensuring subarray size of 1.
            ll md = l+(r-l)/2;
            ll sum = pfx[md] - pfx[i - 1];
            if((sum) <= x) {
                prospective = md;
                l = md + 1;
            } else {
        ...
        if(prospective == -1 or pfx[prospective] - pfx[i - 1] != x) continue;
        // dbg(prospective);
        ans = min(ans, i + (n - prospective - 1));
    }
